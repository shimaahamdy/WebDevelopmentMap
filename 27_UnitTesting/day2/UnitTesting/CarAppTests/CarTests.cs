...
namespace CarAppTests
{
    ...
    public class CarTests
    {
        ...
        [TestMethod]
        public void TimeToCoverProvidedDistance_Velocity0_Infinity()
        {
            var car = new Car(CarType.BMW, 0, DrivingMode.Forward);

            var distanceToBeCovered = 100;
            double actualTime = car.TimeToCoverProvidedDistance(distanceToBeCovered);
            double expectedTime = double.PositiveInfinity;
        ...
        public void Test()
        {
            ...
            var actual = car.GetType();
            var expected = "BMW";

            // it doesn't return, it just returns boolean value. so it is as if no Assert is written
            StringAssert.Contains("", "ay 7aga");
        }
